...
describe 'The Aviary App' do
  ...
  describe 'GET /instances_pinged_aviary' do
    ...
    context 'when canaries are not ok' do
      before { InstancePinger.any_instance.stub(running_ratio: 0.1) }
  ...
  describe 'GET /instances_from_cf_aviary' do
    context 'when canaries are ok' do
      before { InstancesFromCFAviary.any_instance.stub(ok?: true) }

      it 'sings' do
        get '/instances_from_cf_aviary'
        last_response.should be_ok
        last_response.body.should == 'Sing'
      end
    end
